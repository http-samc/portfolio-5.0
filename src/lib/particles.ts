...
const Particles = {
  ...
  init: function () {
    ...
    this.count = Math.floor(
      Math.sqrt(window.innerWidth * window.innerWidth) * 0.25
    );
  ...
  Star: function (x, y, dx, dy, radius, fill) {
    ...
    this.update = function () {
      if (
        this.x + this.radius > Particles.canvas.width ||
        this.x - this.radius < 0
      )
        this.dx = -this.dx;
      if (
        this.y + this.radius > Particles.canvas.height ||
      ...
      if (
        ...
        this.radius = Math.min(this.radius * 1.05, 2.5);
